...
<div class="min-h-screen bg-slate-50/50">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        <div class="mb-6">
            <p class="text-sm text-slate-500 mb-1">Edição de submissão</p>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6">
            ...
            <form action="{{ route('works.update', $work->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="work_type" required
                        ...
                        @foreach($workTypes as $type)
                            <option value="{{ $type }}" @selected(old('work_type', $work->work_type) === $type)>
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Novo arquivo (PDF, DOC, DOCX ou ODT — opcional)</label>
                ...
                <div class="pt-2 border-t border-slate-100">
                    <p class="text-sm text-slate-600 mb-3 m-0">
                        <strong>Autor principal:</strong> {{ $work->submitter->name }} ({{ $work->submitter->email }}).
                    </p>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Outros autores (opcional)</label>
                    <p class="text-xs text-slate-500 mb-3 m-0">Adicione ou edite coautores. O autor principal acima não pode ser alterado aqui.</p>
                    @php
                        $manualRows = old('coauthors_manual');
                        if (!is_array($manualRows)) {
                            $manualRows = $manualCoauthorRows;
                        }
